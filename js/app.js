...
function isComplexFunctionQuestionText(text) {
  var value = String(text || "");
  if (isSingleFunctionGraphText(value)) {
    return false;
  }
...
function isSingleFunctionGraphText(text) {
  ...
  return /(?:画出|作出|绘制).{0,30}函数.{0,80}图[像象]/.test(value)
    && getFunctionDefinitionCount(value) <= 2
    && !isComplexMultiQuestionText(value)
    && !/L\s*1|L\s*2|y\s*1|y\s*2|y\s*3|中心对称|双倍比例点|面积|平行于\s*x\s*轴/i.test(value);
}
...
function hasSampledFunctionGraph(spec) {
  return Boolean(
    spec
    && spec.type === "function_graph"
    && Array.isArray(spec.curves)
    && spec.curves.some(function(curve) {
...
function getRenderableVisualizationViews(spec) {
  return getVisualizationViews(spec).filter(function(view) {
    return view && view.type !== "notice";
  });
}

function resolveVisualizationViewId(spec, requestedViewId) {
  var views = getRenderableVisualizationViews(spec);

  if (requestedViewId && findVisualizationView(spec, requestedViewId)) {
  ...
  return requestedViewId && !findVisualizationView(spec, requestedViewId) ? null : requestedViewId;
}
...
function appendVisualizationForView(container, spec, viewId, fallbackTitle = "图示") {
  ...
  const effectiveViewId = isSingleFunctionGraphText(questionText) && hasSampledFunctionGraph(spec)
    ? null
    : resolveVisualizationViewId(spec, viewId);
  const view = findVisualizationView(spec, effectiveViewId);
  ...
  if (window.MathVisualization?.renderView && effectiveViewId) {
    window.MathVisualization.renderView(board, spec, effectiveViewId, vizOptions);
...
function renderStructuredResult(result) {
  ...
  if (!visualizationShown && hasFormalVisualization) {
    visualizationShown = appendVisualizationForView(flow, visualizationSpec, null, "图示讲解") || visualizationShown;
  }
...
function normalizeEscapedLatexBackslashes(text) {
  var s = String(text || "");
  var commands = "(frac|sqrt|times|cdot|triangle|angle|text|left|right|le|ge|neq|pm|sin|cos|tan)";
  ...
  s = s
    ...
    .replace(/\\\\\[/g, "\\[")
    .replace(/\\\\\]/g, "\\]")
    .replace(new RegExp("\\\\\\\\(?=" + commands + "\\b)", "g"), "\\");

  s = s.replace(/\((\\(?:frac|sqrt|times|cdot|triangle|angle|text|le|ge|neq|pm|sin|cos|tan)[^()\n]{0,160})\)/g, "\\($1\\)");
  s = s.replace(/(^|[^\w\\])([a-zA-Z]\s*=\s*-?\\frac\{[^{}]+\}\{[^{}]+\})/g, function(match, prefix, formula) {
...
function prepareMathTextForDisplay(rawText) {
  ...
  cleaned = normalizeDisplayLatex(cleaned);
