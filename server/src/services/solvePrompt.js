...
function buildBaseSystemPrompt() {
  return [
    ...
    "平行四边形基础性质题必须先确认 ABCD 是平行四边形，再使用初中性质：平行四边形的对边相等、对角相等、对角线互相平分；条件不足时不要强行套性质，不使用坐标法、向量法或高中方法，不把四边形综合题简化成基础平行四边形图。",
    "特殊四边形与梯形中位线题必须先确认图形类型：矩形可用对角线相等，菱形可用对角线互相垂直，梯形中位线平行于两底且等于两底和的一半；条件不足时不要强行套性质，不使用坐标法、向量法或高中方法，不把四边形综合题简化成基础性质图。",
  ].join("\n");
}
...
function buildGeometryUserPrompt({
  ...
  return [
    ...
    "10y. 对“ABCD 是平行四边形，求证 AB=CD，AD=BC”这类题，解析必须先确认 ABCD 是平行四边形，再由平行四边形的对边相等得到两组结论；不要把只有线段相等的题误判为平行四边形性质。",
    "10z. 对“ABCD 是平行四边形，求证 ∠A=∠C，∠B=∠D”这类题，解析必须先确认 ABCD 是平行四边形，再由平行四边形的对角相等得到两组结论；不要把普通角相等或平行线角关系题误判为本性质。",
    "10aa. 对“ABCD 是平行四边形，对角线 AC、BD 交于 O，求证 AO=OC，BO=OD”这类题，解析必须先确认 ABCD 是平行四边形且两条对角线交于 O，再由平行四边形的对角线互相平分得到结论；不要使用坐标法、向量法或高中方法。",
    "10ab. 对“ABCD 是矩形，求证 AC=BD”这类题，解析必须先确认 ABCD 是矩形，再由矩形的对角线相等得到 AC=BD；不要把普通四边形对角线相等或矩形判定题误判为本性质。",
    "10ac. 对“ABCD 是菱形，对角线 AC、BD 交于 O，求证 AC⊥BD”这类题，解析必须先确认 ABCD 是菱形且两条对角线交于 O，再由菱形的对角线互相垂直得到 AC⊥BD；不要把普通四边形对角线垂直题误判为本性质。",
    "10ad. 对“梯形 ABCD 中，AD∥BC，M、N 分别是 AB、CD 的中点，求证 MN∥AD∥BC，且 MN=(AD+BC)/2”这类题，解析必须先确认 ABCD 是梯形、AD∥BC、M 和 N 分别是两腰中点，再由梯形中位线性质得到平行关系与长度关系；不要把三角形中位线题或普通平行线题误判为梯形中位线。",
