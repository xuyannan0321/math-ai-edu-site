...
```text
已知点 A、B、C、D 在 ⊙O 上，∠ACB 与 ∠ADB 同对弧 AB，求证 ∠ACB=∠ADB。
```

验收标准：

- 应触发 `same_arc_equal_inscribed_angles_v1`；
- 显示圆 ⊙O；
- A、B、C、D 均在圆上；
- 显示 AC、BC、AD、BD；
- 用短弧标出弧 AB；
...
```text
已知 ⊙O 中，⌒AB=⌒CD，∠AEB 对⌒AB，∠CFD 对⌒CD，求证 ∠AEB=∠CFD。
```

验收标准：

- 应触发 `equal_arcs_equal_inscribed_angles_v1`；
- 显示圆 ⊙O；
- 显示 A、B、C、D、E、F；
- 显示两条短弧 ⌒AB 与 ⌒CD；
- 显示 ∠AEB 与 ∠CFD 的角弧；
...
```text
已知 ∠AOB 是 ⊙O 中弧 AB 所对的圆心角，∠ACB 是同弧 AB 所对的圆周角，求证 ∠AOB=2∠ACB。
```

验收标准：

- 应触发 `central_angle_double_inscribed_angle_v1`；
- 显示圆 ⊙O 和圆心 O；
- A、B、C 在圆上；
- 显示 OA、OB、AC、BC；
- 显示弧 AB、圆心角 ∠AOB 和圆周角 ∠ACB；
- 解析使用同弧所对圆心角等于圆周角 2 倍；
- 图中不显示定理长句或推导文字；
- 只有 `∠AOB=2∠ACB`、没有圆或同弧条件时不得触发该模板。

### 测试 34：圆周角为直角推出所对弦为直径

输入：

```text
已知 A、B、C 在 ⊙O 上，∠ACB=90°，求证 AB 是 ⊙O 的直径。
```

验收标准：

- 应触发 `right_angle_subtends_diameter_v1`；
- 显示圆 ⊙O 和圆心 O；
- A、B、C 在圆上；
- 显示 AC、BC、AB；
- C 处显示直角标记；
...
- 解析使用圆周角为直角则所对弦为直径；
- 普通直角三角形不得触发该模板；
- 已知 `AB 是 ⊙O 的直径`、求证 `∠ACB=90°` 时仍应触发 `diameter_right_angle_v1`。
...
```text
已知 AB、CD 是 ⊙O 的弦，AB=CD，求证 ⌒AB=⌒CD。
```

验收标准：

- 应触发 `equal_chords_equal_arcs_v1`；
- 显示圆 ⊙O 和圆心 O；
- A、B、C、D 均在圆上；
- 显示弦 AB、CD；
- 显示对应短弧 ⌒AB、⌒CD；
- 解析使用同圆或等圆中等弦对等弧；
- 图中不显示定理长句或长证明文字；
- 只有 `AB=CD`、没有圆或弦条件时不得触发该模板；
- 只有 `AB、CD 是 ⊙O 的弦`、没有等弦条件时不得触发该模板；
- 点标签、弦标签和弧标签不应压线或堆叠。
...
```text
已知 ⊙O 中，⌒AB=⌒CD，求证 AB=CD。
```

验收标准：

- 应触发 `equal_arcs_equal_chords_v1`；
- 显示圆 ⊙O 和圆心 O；
- A、B、C、D 均在圆上；
- 显示两条短弧 ⌒AB、⌒CD；
- 显示对应弦 AB、CD；
...
- 点标签不应压在线段、半径、弦、圆弧或角弧上；
- 圆上的点标签优先显示在圆外侧；
- 圆心 O 标签不应压在半径、弦或角标签上；
- 角标签之间不应重叠，多角同图时需检查 `arcLevel` 分层是否清楚；
